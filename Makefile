...
HTTPS_GIT := https://github.com/tokenfab/tfabd.git
...
draw-deps:
	...
	@goviz -i ./cmd/tfabd -d 2 | dot -Tpng -o dependency-graph.png
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/tokenfab/tfabd
